...
outputcell='A5'; %stores values in file called 'table'. Path defined at end of script!

filename='121215_Series043_MS';%choose name under which to store it in excel sheet!
...
MIJ.start;
MIJ.run('Open...',strcat('path=[N:\MICROSCOPE\Adam\Projects\Cortical ChAT cells\In Situs\test\testimages\',filename,'.tif]'))
...
pause on
pause;
%optimize ROIs and click somewhere in Matlab command window


MIJ.run('Run...','path=[C:\Fiji.app\macros\step2anew.ijm]')
Punctahist=str2num(MIJ.getLog);
%save Overlay Elements as defined in line 85
figure
subplot(3,1,1)
...
folder = uigetdir; %UI to select a folder to save the files from
cd(folder)
save(strcat(filename,'_Puncta'),'Puncta');
save(strcat(filename,'_Punctathresholding'),'Punctathresholding');
save(strcat(filename,'_databin'),'databin');
xlswrite('N:\MICROSCOPE\Adam\Projects\Cortical ChAT cells\In situs\VGAT,ChAT,GAD in ChAT-i-Cre.xls',A,1,outputcell)
