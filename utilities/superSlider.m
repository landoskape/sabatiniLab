...
%   Updates: 
%       - December 23 2013: Small callback bug - line 537 changed from: 
%                           "eval(callback);" to "eval(props.callback);"
%       - September 15 2014: Larger callback fix to conform with the     
%                            standard matlab callback format: 
%                            function(hObject, eventdata, arguments)
%                            
% -*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*- 
...
try % Optional user callback function
   if ~iscell(props.callback)
        feval(props.callback, sliderHandle, []);
   else
       feval(props.callback{1}, sliderHandle, [], props.callback{2:end});
   end
end
